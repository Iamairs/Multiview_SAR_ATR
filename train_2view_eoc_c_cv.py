...
depth = '1111111'
dropout = '000'
dataset_path = project_root + "/dataset/" + f'{name}/'
...
save_weights_path = project_root + f'/result/{name}_{input_view}view_{depth}depth_{batch_size}bz_{dropout}dropout/'
...
def main():
    ...
    # model
    model = SwinTransformer(in_chans=input_view, num_classes=num_classes, embed_dim=64,
                            batch_size=batch_size,  num_heads=(2, 2, 4, 4, 8, 8, 16), window_size=4,
    ...
    for epoch in range(epochs):
        ...
        train_one_epoch(model=model, optimizer=optimizer, data_loader=train_loader, device=device, epoch=epoch,
                        input_view=input_view, feature_dst_path=train_feature_dst_path)
